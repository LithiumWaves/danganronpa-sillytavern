...
function forceTrialToPhase__dedup1(targetPhase) {
    ...
    const phases = trialController.phases;
    const phaseRoute = [
        phases.TRIAL_INTRO,
        phases.DISCUSSION_PRE_DEBATE,
        phases.NONSTOP_INTRO_CUTSCENE,
        phases.NONSTOP_ACTIVE,
        phases.DISCUSSION_POST_DEBATE,
        phases.MINI_GAME,
        phases.TRIAL_RESULT,
    ].filter(Boolean);

    const normalizedTarget = String(targetPhase || "").trim();
    if (!normalizedTarget) {
        return { ok: false, reason: "missing_target" };
    }

    if (normalizedTarget === "finish") {
        const toResult = forceTrialToPhase(phases.TRIAL_RESULT);
        if (!toResult?.ok) return toResult;

        const finishResult = trialController.cancelTrial?.();
        syncTrialPhaseControllers();
        if (!finishResult?.ok) {
            return { ok: false, reason: finishResult?.reason || "finish_failed" };
        }

        return { ok: true, phase: phases.IDLE, reason: "trial_finished" };
    }

    if (normalizedTarget === phases.IDLE) {
        const cancelResult = trialController.cancelTrial?.();
        syncTrialPhaseControllers();
        if (!cancelResult?.ok) {
            return { ok: false, reason: cancelResult?.reason || "cancel_failed" };
        }
        return { ok: true, phase: phases.IDLE, reason: "forced_idle" };
    }

    const targetIndex = phaseRoute.indexOf(normalizedTarget);
    if (targetIndex === -1) {
        return { ok: false, reason: `unknown_target:${normalizedTarget}` };
    }

    const currentState = trialController.getState?.();
    if (currentState?.phase && currentState.phase !== phases.IDLE) {
        const resetResult = trialController.cancelTrial?.();
        if (!resetResult?.ok) {
            return { ok: false, reason: resetResult?.reason || "reset_failed" };
        }
    }

    const startResult = trialController.requestStartFromUi?.({ source: "debug_force" });
    if (!startResult?.started) {
        syncTrialPhaseControllers();
        return { ok: false, reason: startResult?.reason || "start_failed" };
    }

    for (let i = 1; i <= targetIndex; i += 1) {
        const phase = phaseRoute[i];
        const transitionResult = trialController.transitionTo?.(phase, "debug_force_phase");
        if (!transitionResult?.ok) {
            syncTrialPhaseControllers();
            return { ok: false, reason: transitionResult?.reason || `transition_failed:${phase}` };
        }
    }

    syncTrialPhaseControllers();
    return { ok: true, phase: normalizedTarget, reason: "forced" };
}
...
function buildTrialIntroOstControllerInstance() {
    const candidateTracks = buildExtensionPathCandidates()
        .map(basePath => `${basePath}/assets/classtrial/trialunderground.mp3`);
...
var buildTrialIntroOstControllerInstance = window.buildTrialIntroOstControllerInstance || function () {
    const candidateTracks = buildExtensionPathCandidates()
        .map(basePath => `${basePath}/assets/classtrial/trialunderground.mp3`);
...
var createTrialIntroOstController = window.createTrialIntroOstController || function () {
    const candidateTracks = buildExtensionPathCandidates()
        .map(basePath => `${basePath}/assets/classtrial/trialunderground.mp3`);
...
window.createTrialIntroOstController = createTrialIntroOstController;
var buildTrialIntroOstControllerInstance = window.buildTrialIntroOstControllerInstance || createTrialIntroOstController;
window.buildTrialIntroOstControllerInstance = buildTrialIntroOstControllerInstance;
    ...
    if (!controls) {
        ...
        controls.innerHTML = `
            ...
            <div class="trust-debug-buttons" role="group" aria-label="Debug controls">
                ...
                <button type="button" data-trial-debug-phase="trial_intro">TRIAL: INTRO</button>
                <button type="button" data-trial-debug-phase="discussion_pre_debate">TRIAL: DISCUSSION PRE</button>
                <button type="button" data-trial-debug-phase="nonstop_intro_cutscene">TRIAL: NSD CUTSCENE</button>
                <button type="button" data-trial-debug-phase="nonstop_active">TRIAL: NSD ACTIVE</button>
                <button type="button" data-trial-debug-phase="discussion_post_debate">TRIAL: DISCUSSION POST</button>
                <button type="button" data-trial-debug-phase="mini_game">TRIAL: MINI GAME</button>
                <button type="button" data-trial-debug-phase="trial_result">TRIAL: RESULT</button>
                <button type="button" data-trial-debug-phase="finish">TRIAL: FINISH</button>
            </div>
        `;
    }
...
function bindDebugControlEvents() {
    ...
    $(document).on("click.debugControls", "[data-trial-debug-phase]", event => {
        playDebugClickSfx();
        const targetPhase = String(event.currentTarget?.dataset?.trialDebugPhase || "").trim();
        const result = forceTrialToPhase(targetPhase);

        if (!result?.ok) {
            console.warn(`[Dangan][Debug] Failed to force trial phase (${targetPhase}): ${result?.reason || "unknown"}`);
            return;
        }

        console.log(`[Dangan][Debug] Forced trial phase: ${result.phase}`);
    });
...
trialIntroOstController = buildTrialIntroOstControllerInstance();
